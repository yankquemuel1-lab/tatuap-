...
export async function getProgress(): Promise<UserProgress> {
  ...
  if (!data?.progresso || Object.keys(data.progresso).length === 0) return defaultProgress()
  return data.progresso as UserProgress
}
...
export async function getDinamicaProgress(id: string): Promise<DinamicaProgress> {
  const p = await getProgress()
  return p.dinamicas[id] ?? { leituraCompleta: false, quizCompleto: false, quizPontuacao: 0, conquistas: [] }
}

export async function marcarLeituraCompleta(id: string): Promise<void> {
  const p = await getProgress()
  if (!p.dinamicas[id]) {
...
export async function marcarQuizCompleto(id: string, pontuacao: number): Promise<void> {
  const p = await getProgress()
  if (!p.dinamicas[id]) {
    p.dinamicas[id] = { leituraCompleta: false, quizCompleto: false, quizPontuacao: 0, conquistas: [] }
  }
